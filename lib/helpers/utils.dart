...
class Utils{
  ...
  String putCurrencyMask(num? value, {bool showCurrencyLabel = true}) {
    ...
    if (numResidual > 0) {
      formattedNumber += numGroups > 0
          ? splitParts[0].substring(0, numResidual) + '.'
          : splitParts[0].substring(0, numResidual);
    }
    for (int i = 0; i < numGroups; i++) {
      formattedNumber += (i < numGroups - 1)
          ? splitParts[0]
          .substring(numResidual + 3 * i, numResidual + 3 + 3 * i) +
          '.'
          : splitParts[0]
          .substring(numResidual + 3 * i, numResidual + 3 + 3 * i);
    }
  ...
  String generateAleatoryKeyPix() {
    //4c04c042-def5-46cd-bdac-4366fb8754d9
    ...
    for (int i = 1; i <= 36; i++) {
      if (i == 9 || i == 14 || i == 19 || i == 24) {
        ...
        aleatoryKeyPix += generateRandomString(1);
      }
    }


    print('Chave aleatória: $aleatoryKeyPix');
